...
class Href_Model:
    def __init__(self, url_model, number_page):
    ...
    def href_models(self):
        ...
        for i in range(1, self.number_page):
            page = i
            url = f'{domain}/{self.url_model}?p={page}'
            ...
            for avto in avtos:
                href = avto.get('href')
                list_href_cars.append(href)

        with open (f'list_href_cars_{self.url_model}.json', 'w') as f:
            json.dump(list_href_cars, f)
